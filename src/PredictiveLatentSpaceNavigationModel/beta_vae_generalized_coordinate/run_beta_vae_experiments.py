...
DEFAULT_DB_PATH = os.path.join(SCRIPT_DIR, "beta_vae_generalized_coordinate.db")
DEFAULT_CONFIG_DIR = os.path.join(SCRIPT_DIR, "configs")
DEFAULT_PROCESSED_CONFIG_DIR = os.path.join(SCRIPT_DIR, "configs_processed")
DEFAULT_SCHEMA_PATH = os.path.join(SCRIPT_DIR, "beta_vae_generalized_coordinate_schema.sql")
    ...
    try:
        ...
        with sqlite3.connect(db_path) as conn:
            ...
            cursor.execute("""
                           ...
                           FROM sqlite_master
                           WHERE type = 'table'
                             AND name = 'beta_vae_generalized_coordinate_experiments'
                           """)

            if cursor.fetchone():
                print("既存のbeta_vae_generalized_coordinate_experimentsテーブルを発見")
                # テーブル構造の確認
                cursor.execute("PRAGMA table_info(beta_vae_generalized_coordinate_experiments)")
...
def create_tables_directly(cursor):
    ...
    # beta_vae_generalized_coordinate_experimentsテーブル
    cursor.execute("""
    ...
    # インデックス作成
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_experiments_status ON beta_vae_generalized_coordinate_experiments(status)")
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_experiments_created_at ON beta_vae_generalized_coordinate_experiments(created_at)")
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_experiments_tags ON beta_vae_generalized_coordinate_experiments(tags)")
    cursor.execute(
...
def add_missing_columns(cursor, existing_columns):
    ...
    for column_name, column_def in required_columns.items():
        if column_name not in existing_columns:
            try:
                cursor.execute(f"ALTER TABLE beta_vae_generalized_coordinate_experiments ADD COLUMN {column_name} {column_def}")
...
def register_experiment(db_path: str, config: Dict[str, Any], config_filename: str, config_path: str) -> int:
    ...
    try:
        with sqlite3.connect(db_path) as conn:
            ...
            query = f"INSERT INTO beta_vae_generalized_coordinate_experiments ({columns}) VALUES ({placeholders})"
...
def run_experiment_subprocess(experiment_id: int, config_path: str, db_path: str):
    ...
    """
    print(f"--- BetaVAE実験 {experiment_id} をサブプロセスとして開始 ---")
    ...
    train_script_path = os.path.join(SCRIPT_DIR, 'train_beta_vae_generalized_coordinate.py')
    ...
    try:
        ...
        print(f"--- BetaVAE実験 {experiment_id} 正常終了 ---")
        ...
        print(f"!!! エラー: BetaVAE実験 {experiment_id} が失敗しました !!!")
...
def update_experiment_status(db_path: str, experiment_id: int, status: str, **kwargs):
    ...
    try:
        with sqlite3.connect(db_path) as conn:
            ...
            query = f"UPDATE beta_vae_generalized_coordinate_experiments SET {', '.join(update_fields)} WHERE id = ?"
        ...
        with sqlite3.connect(db_path) as conn:
            ...
            cursor.execute("""
                           SELECT id,
                                  ...
                                  end_time,
                                  tags
                           FROM beta_vae_generalized_coordinate_experiments
...
def main(args):
    ...
    print(f"\n{len(config_files)}件のBetaVAE実験を開始します:")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="BetaVAE実験管理システム")
