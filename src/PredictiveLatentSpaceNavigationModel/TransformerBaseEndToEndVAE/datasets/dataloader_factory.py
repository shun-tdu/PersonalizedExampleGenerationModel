...
from .factor_score_metrics_dataset import FactorScoreMetricsDataset, FactorScoreMetricsNoInterpolateDataset, collate_fn_padd
...
class DatasetRegistry:
    ...
    _datasets = {
        ...
        'factor_score_metrics': FactorScoreMetricsDataset,      # 因子スコア入りデータセット [batch, trajectory, subject_id, skill_score. factor_1,..., factor_n]
        'factor_score_metrics_no_interpolate': FactorScoreMetricsNoInterpolateDataset,   # 因子スコア入りデータセット [batch, trajectory, subject_id, skill_score. factor_1,..., factor_n] 時間補完なし
...
class DataLoaderFactory:
    ...
    def create_dataloaders(config: Dict[str, Any]) -> Tuple[DataLoader, Optional[DataLoader], DataLoader, None, Dataset, Optional[Dataset], Dataset]:
        ...
        elif dataset_type == 'factor_score_metrics_no_interpolate':
            return DataLoaderFactory._create_factor_score_metrics_no_interpolate_dataloaders(data_config, training_config)
        else:
    ...
    @staticmethod
    def _create_factor_score_metrics_no_interpolate_dataloaders(data_config: Dict[str, Any], training_config: Dict[str, Any]) -> Tuple[
        DataLoader, Optional[DataLoader], DataLoader, pd.DataFrame, Dataset, Optional[Dataset], Dataset]:
        """スキル因子指標データローダーを作成"""
        print("スキル因子指標データローダーを作成中...")
        ...
        # Datasetの作成
        train_dataset = FactorScoreMetricsNoInterpolateDataset(train_df, scalers, feature_cols, data_config)
        val_dataset = FactorScoreMetricsNoInterpolateDataset(val_df, scalers, feature_cols,
                                                data_config) if not val_df.empty else None
        test_dataset = FactorScoreMetricsNoInterpolateDataset(test_df, scalers, feature_cols, data_config)
