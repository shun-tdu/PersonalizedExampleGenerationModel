...
class ImprovedStyleSkillSeparationNetDecoder(nn.Module):
    ...
    def forward(self, z_style, z_skill, skip_connections):
        ...
        # 潜在空間から生成した大局的な情報
        memory_from_z = fused_features.unsqueeze(1).expand(-1, self.seq_len, -1) # [Batch, seq_len, d_model]
        ...
        for i, layer in enumerate(self.decoder_layers):
            ...
            # 潜在変数とスキップ接続の情報を融合(Gate Unity)
            gate_input = torch.cat([decoded_features, skip], dim=2)
            g = self.gating_layers[i](gate_input)
            fused_input = g * skip + (1 - g) * decoded_features
...
class ImprovedStyleSkillSeparationNet(BaseExperimentModel):
    def __init__(self,
        ...
        print(f"GatedSkipConnectionStyleSkillSeparationNet instantiated with:")
    ...
    def decode(self, z_style, z_skill, skip_connections=None):
        ...
        Args:
            ...
            skip_connections: エンコーダからのスキップ接続リスト（Noneの場合はダミー生成）
        """
        if skip_connections is None:
            # エンコーダが無い場合はゼロテンソルでスキップ接続を代用
            batch_size = z_style.size(0)
            dummy_skip = torch.zeros(batch_size, self.seq_len, self.d_model, device=z_style.device)
            # デコーダ層数分のダミースキップ接続を生成
            n_decoder_layers = len(self.decoder.decoder_layers)
            skip_connections = [dummy_skip.clone() for _ in range(n_decoder_layers)]
