...
class FilmLayer(nn.Module):
    def __init__(self, condition_dim, feature_channels):
        ...
        Args:
            ...
            feature_channels: 変調される特徴マップのチャンネル数
        """
        super().__init__()
...
class AdaptiveGatedSkipFiLMDecoder(nn.Module):
    def __init__(self,
                 output_dim,
                 seq_len,
                 d_model,
        ...
        # ゲート損失記録
        self.register_buffer('bottleneck_loss', torch.zeros(n_layers))
    ...
    def forward(self, z_style, z_skill, skip_connections, epoch=0, max_epochs=200):
        ...
        for i, (layer, film, gate, bottleneck) in enumerate(
                zip(self.decoder_layers, self.film_layers, self.adaptive_gates, self.information_bottlenecks)
        ):
            # Transformer処理
            ...
            # Adaptive Gate計算
            gate_weights = gate(filmed_features, processed_skip, temperature)
            ...
            # 重要: 次の層のために結果を更新
            decoded_features = fused_input

        # ボトルネック損失記録
        self.bottleneck_losses = torch.stack(bottleneck_losses)

        # 出力（最終的なdecoded_featuresを使用）
...
class FilmAdaptiveGatedSkipConnectionNet(BaseExperimentModel):
    ...
    def __init__(self,
        ...
        self.decoder = AdaptiveGatedSkipFiLMDecoder(
