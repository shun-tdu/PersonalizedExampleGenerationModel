...
try:
    ...
    from train_hierarchical_vae import TrajectoryDataset, create_dataloaders
except ImportError as e:
    print(f"インポートエラー: {e}")
    print("階層型VAEモジュールが見つかりません")
    sys.exit(1)
...
class LatentSpaceVisualizer:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                experiment_info = {
                    ...
                    'reconstruction_mse': result[5],
                    'style_separation_score': result[6],
                    'skill_performance_correlation': result[7],
                    'data_path': result[8],
    ...
    def _load_model(self):
        ...
        model = HierarchicalVAE(**self.config['model'])
    ...
    def _create_comparison_plot(self, latent_arrays, metadata, colors):
        ...
        plt.suptitle('Hierarchical VAE Latent Space Comparison (PCA)', fontsize=16, fontweight='bold')
        plt.tight_layout()
...
def main():
    ...
    parser = argparse.ArgumentParser(description="階層型VAE潜在空間可視化システム")
