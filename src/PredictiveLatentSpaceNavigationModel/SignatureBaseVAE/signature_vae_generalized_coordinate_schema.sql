...
-- インデックス作成
CREATE INDEX IF NOT EXISTS idx_experiments_status ON signature_vae_experiments(status);
CREATE INDEX IF NOT EXISTS idx_experiments_created_at ON signature_vae_experiments(created_at);
CREATE INDEX IF NOT EXISTS idx_experiments_tags ON signature_vae_experiments(tags);
...
CREATE TRIGGER IF NOT EXISTS update_experiments_timestamp
    AFTER UPDATE ON signature_vae_experiments
    FOR EACH ROW
BEGIN
    UPDATE signature_vae_experiments
