...
DEFAULT_DB_PATH = os.path.join(SCRIPT_DIR, "signature_vae_generalized_coordinate.db")
DEFAULT_CONFIG_DIR = os.path.join(SCRIPT_DIR, "configs")
DEFAULT_PROCESSED_CONFIG_DIR = os.path.join(SCRIPT_DIR, "configs_processed")
DEFAULT_SCHEMA_PATH = os.path.join(SCRIPT_DIR, "signature_vae_generalized_coordinate_schema.sql")
    ...
    try:
        ...
        with sqlite3.connect(db_path) as conn:
            ...
            cursor.execute("""
                           ...
                           FROM sqlite_master
                           WHERE type = 'table'
                             AND name = 'signature_vae_experiments'
                           """)

            if cursor.fetchone():
                print("既存のsignature_vae_experimentsテーブルを発見")
                # テーブル構造の確認
                cursor.execute("PRAGMA table_info(signature_vae_experiments)")
            ...
            conn.commit()
            print("SignatureVAE実験データベースの準備完了")
...
def create_tables_directly(cursor):
    ...
    cursor.execute("""
        ...
        VALUES ('signature_vae_experiments', '1.0', 'Signature VAE experiment tracking database')
    """)

    # signature_vae_generalized_coordinate_experimentsテーブル
    cursor.execute("""
                   CREATE TABLE IF NOT EXISTS signature_vae_experiments
                   (
                       -- 基本情報
                       id
                       INTEGER
    ...
    # インデックス作成
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_experiments_status ON signature_vae_experiments(status)")
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_experiments_created_at ON signature_vae_experiments(created_at)")
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_experiments_tags ON signature_vae_experiments(tags)")
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_experiments_performance ON signature_vae_experiments(reconstruction_mse)")
...
def add_missing_columns(cursor, existing_columns):
    ...
    for column_name, column_def in required_columns.items():
        if column_name not in existing_columns:
            try:
                cursor.execute(f"ALTER TABLE signature_vae_experiments ADD COLUMN {column_name} {column_def}")
...
def register_experiment(db_path: str, config: Dict[str, Any], config_filename: str, config_path: str) -> int:
    ...
    try:
        with sqlite3.connect(db_path) as conn:
            ...
            query = f"INSERT INTO signature_vae_experiments ({columns}) VALUES ({placeholders})"
...
def run_experiment_subprocess(experiment_id: int, config_path: str, db_path: str):
    ...
    train_script_path = os.path.join(SCRIPT_DIR, 'train_signature_vae_generalized_coordinate.py')
...
def update_experiment_status(db_path: str, experiment_id: int, status: str, **kwargs):
    ...
    try:
        with sqlite3.connect(db_path) as conn:
            ...
            query = f"UPDATE signature_vae_experiments SET {', '.join(update_fields)} WHERE id = ?"
        ...
        with sqlite3.connect(db_path) as conn:
            ...
            cursor.execute("""
                           SELECT id,
                                  ...
                                  end_time,
                                  tags
                           FROM signature_vae_experiments
