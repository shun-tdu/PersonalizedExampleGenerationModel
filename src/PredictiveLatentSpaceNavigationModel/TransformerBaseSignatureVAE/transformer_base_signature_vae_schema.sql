...
-- インデックス作成
CREATE INDEX IF NOT EXISTS idx_experiments_status ON transformer_base_signature_vae_experiment(status);
CREATE INDEX IF NOT EXISTS idx_experiments_created_at ON transformer_base_signature_vae_experiment(created_at);
CREATE INDEX IF NOT EXISTS idx_experiments_tags ON transformer_base_signature_vae_experiment(tags);
...
CREATE TRIGGER IF NOT EXISTS update_experiments_timestamp
    AFTER UPDATE ON transformer_base_signature_vae_experiment
    FOR EACH ROW
BEGIN
    UPDATE transformer_base_signature_vae_experiment
