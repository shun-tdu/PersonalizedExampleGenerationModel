...
DEFAULT_DB_PATH = os.path.join(os.path.dirname(SCRIPT_DIR), "experiments.db")
...
def register_experiment(db_path: str, config: Dict[str, Any], config_filename: str) -> int:
    ...
    params = {
        ...
        'low_freq_model_type': config.get('model', {}).get('architecture', {}).get('low_freq_model'),
        'high_freq_model_type': config.get('model', {}).get('architecture', {}).get('high_freq_model'),
        'lr_low_freq': config.get('training', {}).get('lr_low_freq'),
        'lr_high_freq': config.get('training', {}).get('lr_high_freq'),
        'batch_size': config.get('training', {}).get('batch_size'),
        'num_epochs': config.get('training', {}).get('num_epochs'),
        'transformer_layers': config.get('model', {}).get('low_freq_params', {}).get('num_layers'),
        'transformer_dim': config.get('model', {}).get('low_freq_params', {}).get('inner_dim'),
    }
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        curses.execute(query, tuple(params.values()))
        experiment_id = curses.lastrowid
        conn.commit()
