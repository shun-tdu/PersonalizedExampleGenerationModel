...
# #         # 畳み込みのチャンネル数がAttentionの次元(query_dim)になる
...
# #                 nn.Conv1d(out_c, out_c, kernel_size=3, stride=2,
# #                           padding=1) if i < num_resolutions - 1 else nn.Identity()
# #             )
# #             in_c = out_c
# #
# #         # Bottleneck
# #         self.bottleneck = ResidualBlock(channels[-1], channels[-1], time_embed_dim, condition_dim)
# #
# #         # Decoder(Up-sampling)
...
# #                 )
# #                 decoder_in_channels = in_c + out_c
# #             else:
# #                 self.upsample_layers.append(nn.Identity())
# #                 decoder_in_channels = in_c
# #
# #             self.decoder_blocks.append(ResidualBlock(decoder_in_channels, out_c, time_embed_dim, condition_dim))
# #             in_c = out_c
# #
# #         # 出力層
...
# #
# #         # Decoder
# #         for block, upsample in zip(self.decoder_blocks, self.upsample_layers):
# #             x = upsample(x)
# #
# #             if len(skip_connections) > 0:
# #                 skip = skip_connections.pop()
# #                 # サイズが異なる場合はアダプティブにリサイズ
...
#
#         in_c = channels[-1]
#         reversed_channels = list(reversed(channels[:-1]))  # ボトルネック以外を逆順に
#
...
#                                    padding=1, output_padding=1)
#             )
#             # スキップ接続を考慮したチャンネル数（アップサンプル後 + スキップ接続）
#             decoder_in_channels = out_c + out_c  # アップサンプル結果 + スキップ接続（同じチャンネル数）
#
#             self.decoder_blocks.append(ResidualBlock(decoder_in_channels, out_c, time_embed_dim, condition_dim))
#             in_c = out_c
#
...
#         # Bottleneck
#         x = self.bottleneck(x, time_embed, condition)
#         skip_connections.pop()  # ボトルネックのスキップ接続は使わない
#
#         # Decoder
#         for i, (block, upsample) in enumerate(zip(self.decoder_blocks, self.upsample_layers)):
#             x = upsample(x)
#
#             # スキップ接続を追加
#             skip = skip_connections.pop()
